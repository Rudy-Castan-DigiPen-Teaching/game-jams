...
function PreLoadSounds() {
  ...
  StartSounds = [
    ...
    loadSound("audio/lets_go_03.ogg"),
...
function PlayStartSound() {
  const sfx = this.StartSounds[
    Math.floor(Math.random() * this.StartSounds.length)
  ];
  setTimeout(() => {
    sfx.play();
...
function PlayHurtSound() {
  const sfx = this.HurtSounds[
    Math.floor(Math.random() * this.HurtSounds.length)
  ];
  sfx.play();
}

function PlayEatSound() {
  const sfx = this.EatItemSounds[
    Math.floor(Math.random() * this.EatItemSounds.length)
  ];
  sfx.play();
}
